 ...
 * @version 1.0.0
 */
public class Httpmq implements Runnable {
	private HttpmqApi httpmqApi = null;
	private CanalConnector connector = null;
	private String thread_name = null;
	private String canal_destination = null;

	public Httpmq(String name) {
		...
		for (Entry entry : entrys) {
			...
			for (RowData rowData : rowChage.getRowDatasList()) {
				...
				try {
					...
					WriteLog.write(canal_destination + ".error", thread_name + "httpmq link failure!" + WriteLog.eString(e));
					ret = false;
				}
			}
			data.clear();
